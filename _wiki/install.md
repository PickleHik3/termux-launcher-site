...
1.  **Pin Apps to Dock:** Long press on the empty space in the dock to pin your favorite apps.
2. **Quick reply to notifications:** for pinned apps which has an unread notification, you can swipe up on the app icon to respond to the message. It works using the "reply" field available on the app's notification. 
3. **Most used apps page:** enables an additional page at the end of pinned app pages showing your most frequent non-pinned apps. Toggle it from Setting > Launcher & Apps
4. **Double tap the Alphabets Row to Lock screen:** You have 2 options;
...
5. **App Launching:** you have 3 options;

   * Tap on an alphabet, to see filtered app icons, normal tap opens it. 
...
6. **Command Palette:** The app features a command palette triggered by either "Ctrl + Shift + Alt + P" or swipe up from the space bar key of in-app keyboard. 

   * Launch apps
   * Shell controls 
   * Read about the command palette in more detail on the [Command Palette](#wiki/tour) page.
7. **App actions on hold:** long press any app icon - in the dock or in the filtered row - for its Android shortcuts plus *App info*, *Uninstall*, *Change app icon*, *Change dock icon* and *Unpin*.
