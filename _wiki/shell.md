...
| **Termux Launcher tmux plugin** | Material themes, Android-friendly bindings, and status widgets |
| **eza** | Colored, icon-aware file listings and tree views |
| **zoxide** | Learns frequently used directories for fast jumps |
| **launcherctl** | Launches Android apps |
...
Add only the bindings you want to `~/.tmux.conf`:

```tmux
bind -n M-m run-shell 'launcherctl launch maps >/dev/null 2>&1 || tmux display-message "Launch failed: Maps"'
```

In tmux syntax, `M-m` means Alt+m. If a label matches several apps, use the package or activity instead. Avoid copying a large personal binding list: start with one app, test it, then add more.
...
The guarded installer fast-forwards a clean Termux Launcher tmux plugin checkout. It stops if the checkout has local changes and leaves existing fish, Oh My Posh, and tmux configuration files in place. The second command reloads your tmux config.
